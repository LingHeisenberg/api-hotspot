...
// Expiration ends network access, not the sale. The expiration service only
// transitions paid/used vouchers, so their payment remains approved.
const approved="v.status IN ('pago','usado','expirado')";
// Legacy sales without a confirmation timestamp retain their reservation date.
const paymentDate='COALESCE(v.pago_em,v.reservado_em)';
const fields=()=>`v.id,v.transacao_id AS reference,v.telefone_cliente AS phone,p.nome AS package_name,p.preco AS amount,v.payment_provider AS method,v.status,v.codigo_voucher AS voucher,v.reservado_em AS created_at,v.pago_em AS confirmed_at,${maputoColumn(paymentDate)} AS payment_date,v.usado_em AS used_at,v.mikrotik_login_at AS activated_at,v.ip_cliente AS ip,v.mac_cliente AS mac,v.mikrotik_sync_status,v.mikrotik_sync_erro,v.mikrotik_login_message`;
export function paymentFilter(query={},receipts=false) {
  const where=['v.transacao_id IS NOT NULL'], args=[];
  if(receipts) where.push(approved);
  if(query.from||query.to||query.period) {const r=dateRange(query);where.push(`${maputoColumn(paymentDate)}>=? AND ${maputoColumn(paymentDate)}<?`);args.push(r.start,r.end);}
...
export async function payments(query,receipts=false) {
  ...
  const column={date:paymentDate,amount:'p.preco',status:'v.status'}[query.sort||'date'];
  if(!column||!['asc','desc'].includes(query.direction||'desc')) fail(422,'Ordenação inválida.');
  return {...await paginated(pool,`SELECT ${fields()}`,from,args,query,`${column} ${(query.direction||'desc').toUpperCase()},v.id DESC`)};
}
export async function paymentDetail(id,receipt=false) {
  const [rows]=await pool.execute(`SELECT ${fields()} FROM vouchers v JOIN pacotes p ON p.id=v.pacote_id WHERE v.id=? AND v.transacao_id IS NOT NULL ${receipt?'AND '+approved:''}`,[id]);
  ...
  return {payment:{...payment,provider_reference:refs[0]?.provider_reference||null},events,receiptNumber:receipt?`EY-${String(payment.id).padStart(8,'0')}`:undefined};
}
export async function report(query={}) {
  const range=dateRange(query), {from,args}=paymentFilter({...query,from:range.from,to:range.to});
  const [[summary]]=await pool.execute(`SELECT COUNT(*) AS payments,COALESCE(SUM(${approved}),0) AS approved,COALESCE(SUM(v.status='pendente'),0) AS pending,COALESCE(SUM(v.status='cancelado'),0) AS canceled,COALESCE(SUM(IF(${approved},p.preco,0)),0) AS revenue,COALESCE(AVG(IF(${approved},p.preco,NULL)),0) AS average ${from}`,args);
  const [packages]=await pool.execute(`SELECT p.nome AS name,COUNT(*) AS payments, SUM(${approved}) AS approved, SUM(IF(${approved},p.preco,0)) AS revenue ${from} GROUP BY p.id,p.nome ORDER BY revenue DESC`,args);
  const [daily]=await pool.execute(`SELECT DATE(${maputoColumn(paymentDate)}) AS day, COUNT(*) AS payments,SUM(IF(${approved},p.preco,0)) AS revenue ${from} GROUP BY day ORDER BY day`,args);
  ...
  const [hours]=await pool.execute(`SELECT HOUR(${maputoColumn(paymentDate)}) AS hour,COUNT(*) AS sales ${from} AND ${approved} GROUP BY hour ORDER BY hour`,args);
  ...
  return {range,summary,packages,daily,states,methods,hours,vouchers,trials};
}
export async function dashboard() {
  const today=await report({period:'today'}),week=await report({period:'week'}),month=await report({period:'month'});
  const [[total]]=await pool.execute(`SELECT COALESCE(SUM(p.preco),0) AS revenue FROM vouchers v JOIN pacotes p ON p.id=v.pacote_id WHERE v.transacao_id IS NOT NULL AND ${approved}`);
  ...
  return {today:today.summary,week:week.summary,month:month.summary,total,stock,trials,charts:await report({from,to}),reconciliation,activeClients};
}
